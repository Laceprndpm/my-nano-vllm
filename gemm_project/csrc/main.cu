...
std::vector<float>
cpu_sgemm_ref(const std::vector<float>& A, const std::vector<float>& B, int M, int N, int K) {
...
BenchmarkResult run_naive(int M, int N, int K, int warmup, int iters, bool verify) {
...
BenchmarkResult run_smem(int M, int N, int K, int warmup, int iters, bool verify) {
    size_t bytes_a = static_cast<size_t>(M) * K * sizeof(float);
    size_t bytes_b = static_cast<size_t>(K) * N * sizeof(float);
    size_t bytes_c = static_cast<size_t>(M) * N * sizeof(float);
...
BenchmarkResult run_tile(int M, int N, int K, int warmup, int iters, bool verify) {
    // fast path only
