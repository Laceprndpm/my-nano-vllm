...
__global__ void
hgemm_t_8x8_sliced_k_f16_kernel(const half* a, const half* b, float* c, int M, int N, int K) {
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int tx = threadIdx.x;
    int ty = threadIdx.y;
    ...
    for (int bk = 0; bk < (K + BK - 1) / BK; ++bk) {
        ...
        HALF2(s_a[load_smem_a_m][load_smem_a_k + 0]) = CHALF2(a[load_gmem_a_addr + 0]);
        HALF2(s_a[load_smem_a_m][load_smem_a_k + 2]) = CHALF2(a[load_gmem_a_addr + 2]);

        HALF2(s_b[load_smem_b_k][load_smem_b_n + 0]) = CHALF2(b[load_gmem_b_addr + 0]);
        HALF2(s_b[load_smem_b_k][load_smem_b_n + 2]) = CHALF2(b[load_gmem_b_addr + 2]);
...
#pragma unroll
                for (int n = 0; n < TN; ++n) {
                    ...
                    r_c[m][n] +=
                        __half2float(s_a[comp_smem_a_m][k]) * __half2float(s_b[k][comp_smem_b_n]);
                }
            }
        }
...
template <typename T, typename U, int kTileM, int kTileN, int kTileK, typename TiledMMA>
__global__ void gemm_simple(T* Cptr, const U* Aptr, const U* Bptr, int m, int n, int k) {
    Tensor A = make_tensor(make_gmem_ptr(Aptr), make_shape(m, k), make_stride(k, Int<1>{}));
    Tensor B = make_tensor(make_gmem_ptr(Bptr), make_shape(n, k), make_stride(Int<1>{}, n));
...
void launch_hgemm_tile(const half* dA, const half* dB, float* dC, int M, int N, int K) {
    ...
    hgemm_t_8x8_sliced_k_f16_kernel<BM, BN, BK, TM, TN><<<grid, block>>>(dA, dB, dC, M, N, K);
}
...
void launch_hgemm_cute(const half* dA, const half* dB, float* dC, int M, int N, int K) {
    ...
    using MMA = decltype(make_tiled_mma(
        mma_atom{}, make_layout(Shape<_4, _4, _1>{}), make_layout(Shape<_1, _2, _1>{})));
    ...
    gemm_simple<T, U, kTileM, kTileN, kTileK, MMA><<<grid, block>>>(dC, dA, dB, M, N, K);
}