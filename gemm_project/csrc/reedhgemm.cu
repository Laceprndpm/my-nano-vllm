...
template <typename T> struct ComputeTypeTraits {
...
template <> struct ComputeTypeTraits<float> {
...
template <typename T, typename ComputeType = T> struct CublasLtGemm {
    ...
    static constexpr cudaDataType_t kScaleType = ComputeTypeTraits<ComputeType>::kScaleType;
...
template <typename T, typename ComputeType> bool CublasLtGemm<T, ComputeType>::run() {
...
void CublasLtGemm<T, ComputeType>::init(T* c, const T* a, const T* b, int m, int n, int k) {
    ...
    check(cublasLtMatrixLayoutSetAttribute(
        a_desc_, CUBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &a_stride, sizeof(a_stride)));
    check(cublasLtMatrixLayoutSetAttribute(
        b_desc_, CUBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &b_stride, sizeof(b_stride)));
    check(cublasLtMatrixLayoutSetAttribute(
        c_desc_, CUBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, &c_stride, sizeof(c_stride)));
    ...
    cublasLtMatmulAlgoGetHeuristic(handle_,
                                   matmul_desc_,
                                   a_desc_,
                                   b_desc_,
                                   c_desc_,
...
template <typename T> void cpu_rand_data(T* c);

template <typename T> void cpu_const_data(T* c, float k);

template <typename T> void cpu_gemm(T* c, const T& a, const T& b);

template <typename T> void cpu_compare(const T& x, const T& y, float threshold = 1.E-1);

template <typename T> void gpu_compare(const T* x, const T* y, int n, float threshold = 1.E-1);
...
template <typename T> void cpu_compare(const T& x, const T& y, float threshold) {
    using namespace cute;
...
template <typename T>
__global__ void
gpu_compare_kernel(const T* x, const T* y, int n, float threshold, int* count, float* max_error) {
...
template <typename T> void gpu_compare(const T* x, const T* y, int n, float threshold) {
    int* num_count;
    float* max_error;
...
static bool split_key_and_val(std::string* key, std::string* val, const std::string& kv_and_eq) {
...
gemm_multi_stage(void* Dptr, const void* Aptr, const void* Bptr, int m, int n, int k) {
    ...
    Tensor A =
        make_tensor(make_gmem_ptr((T*) Aptr), make_shape(m, k), make_stride(k, Int<1>{})); // (M, K)
    Tensor B =
        make_tensor(make_gmem_ptr((T*) Bptr), make_shape(n, k), make_stride(k, Int<1>{})); // (N, K)
    Tensor D =
        make_tensor(make_gmem_ptr((T*) Dptr), make_shape(m, n), make_stride(n, Int<1>{})); // (M, N)

    // slice the tensor to small one which is used for current thread block.
    Tensor gA = local_tile(
        A, make_tile(Int<kTileM>{}, Int<kTileK>{}), make_coord(iy, _)); // (kTileM, kTileK, k)
    Tensor gB = local_tile(
        B, make_tile(Int<kTileN>{}, Int<kTileK>{}), make_coord(ix, _)); // (kTileN, kTileK, k)
    Tensor gD = local_tile(
        D, make_tile(Int<kTileM>{}, Int<kTileN>{}), make_coord(iy, ix)); // (kTileM, kTileN)
...
struct GemmConfig {
    ...
    using SmemLayoutC = decltype(tile_to_shape(
        SmemLayoutAtomC{}, make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}, Int<kSmemLayoutCBatch>{})));
...
int main(int argc, char* argv[]) {
    ...
    for (int it = 0; it < nt; ++it) {
        // blas
        cudaMemset(Dptr_cublas, 0, sizeof(T) * M * N);
        cublasStatus_t ret = cublasHgemm(handle,
                                         CUBLAS_OP_T,
                                         CUBLAS_OP_N,
                                         N,
                                         M,
    ...
    cudaMemcpy(Dptr_host_cublaslt, Dptr_cublaslt, sizeof(T) * M * N, cudaMemcpyDeviceToHost);
    ...
    auto tD_host_cpu = make_tensor(Dptr_host_cpu, make_shape(M, N), make_stride(N, 1));
    auto tD_host_blas = make_tensor(Dptr_host_blas, make_shape(M, N), make_stride(N, 1));
    auto tD_host_cublaslt = make_tensor(Dptr_host_cublaslt, make_shape(M, N), make_stride(N, 1));
