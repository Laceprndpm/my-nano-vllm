...
def _build_dense_varlen_case(seq_len: int):
    batch = 2
    device = "cuda"
...
def _build_paged_varlen_case(seq_len: int):
    batch = 2
    device = "cuda"
    ...
    q, dense_k, dense_v, cu_q, cu_k = _build_dense_varlen_case(seq_len)
...
def test_varlen_man_matches_flash_attn_for_dense_kv(seq_len):
    _require_cuda()
    _seed()

    q, k, v, cu_q, cu_k = _build_dense_varlen_case(seq_len)
...
@pytest.mark.parametrize("seq_len", ALIGNED_SIZES)
def test_varlen_man_matches_official_path_for_paged_kv(seq_len):
    _require_cuda()
    _seed()

    q, k, v, cu_q, cu_k, block_table = _build_paged_varlen_case(seq_len)
    out_man = fa2_varlen_fwd_man(
        q,
        k,
        v,
