...
template <typename T>
__device__ inline T load_global(const T* ptr) {
  return *ptr;
}

template <>
__device__ inline cutlass::half_t load_global(const cutlass::half_t* ptr) {
  return *ptr;
}

template <>
__device__ inline cutlass::bfloat16_t load_global(const cutlass::bfloat16_t* ptr) {
  return *ptr;
}
...
inline __device__ Element load_q_elem(const Params& params,
                                      int q_start,
                                      int q_head_idx,
                                      int q_local,
                                      int d) {
...
template <typename Element, typename Params>
inline __device__ Element load_dense_kv_elem(const void* kv_ptr,
                                             const Params& params,
                                             int k_start,
                                             int kv_head_idx,
                                             int key_local,
...
inline __device__ bool load_paged_kv_elem(const void* kv_ptr,
  ...
  const int32_t block_id = params.block_table_ptr[batch_idx * params.block_table_cols + block_pos];
  if (block_id < 0) {
    return false;
  }
...
__global__ void fa2_varlen_fwd_kernel(const Params params) {
  ...
  const int n_block_max = Is_causal ? cute::ceil_div((m_block + 1) * kBlockM + (k_len - q_len), kBlockN)
    ...
    for (int idx = tidx; idx < kBlockN * kHeadDim; idx += kNThreads) {
      ...
      if (key_local < k_len) {
        if constexpr (Is_paged) {
          Element tmp_k;
          Element tmp_v;
          bool ok_k = load_paged_kv_elem<Element>(params.k_ptr, params, batch_idx, kv_head_idx, key_local, col, tmp_k);
          bool ok_v = load_paged_kv_elem<Element>(params.v_ptr, params, batch_idx, kv_head_idx, key_local, col, tmp_v);
          k_val = ok_k ? tmp_k : Element(0);
          v_val = ok_v ? tmp_v : Element(0);
        } else {
          k_val = load_dense_kv_elem<Element>(params.k_ptr, params, k_start, kv_head_idx, key_local, col);
          v_val = load_dense_kv_elem<Element>(params.v_ptr, params, k_start, kv_head_idx, key_local, col);
        }
      }
      sK(row, col) = k_val;
      sV(row, col) = v_val;
  ...
  for (int idx = tidx; idx < kBlockM * kHeadDim; idx += kNThreads) {
    ...
    const int q_local = m_block * kBlockM + row;
    if (q_local < q_len) {
      const int64_t out_index = static_cast<int64_t>(q_start + q_local) * params.out_row_stride +
...
void set_params_varlen_fwd(VarlenFwdParams& params,
  ...
  params.block_table_ptr = params.paged_kv ? reinterpret_cast<int32_t*>(block_table.data_ptr<int32_t>()) : nullptr;
}
...
void launch_fa2_varlen_fwd(VarlenFwdParams& params, cudaStream_t stream) {
  ...
  const int num_m_block = (params.max_seqlen_q + Kernel_traits::kBlockM - 1) / Kernel_traits::kBlockM;
...
void dispatch_fa2_varlen_fwd(VarlenFwdParams& params, cudaStream_t stream) {
  BOOL_SWITCH(params.is_causal, Is_causal, [&] {
    BOOL_SWITCH(params.paged_kv, Is_paged, [&] {
      launch_fa2_varlen_fwd<
          Flash_fwd_kernel_traits<Headdim, /*kBlockM_=*/64, /*kBlockN_=*/64, /*kNWarps_=*/4, T>,
          Is_causal,
...
torch::Tensor fa2_varlen_fwd_cuda_impl(
  ...
  if (paged_kv) {
    ...
    TORCH_CHECK(k.size(2) == q.size(2) && v.size(2) == q.size(2),
  ...
  nanovllm_fa2_varlen::set_params_varlen_fwd(
      params,
      q_,
      k_,
      v_,
