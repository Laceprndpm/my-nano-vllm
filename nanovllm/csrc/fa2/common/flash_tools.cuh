...
template <typename Tensor0,
          ...
          typename ThrCopyA,
          typename ThrCopyB>
inline __device__ void gemm_smem(Tensor0& acc,
                                 Tensor1& tCrA,
                                 Tensor2& tCrB,
                                 Tensor3 const& tCsA,
                                 Tensor4 const& tCsB,
...
copy(TiledCopy tiled_copy, Tensor<Engine0, Layout0> const& S, Tensor<Engine1, Layout1>& D) {
...
// Convert rowcol_layout from (nrow=(2, MMA_M), ncol=(2, MMA_N)) to A-reg
// layout.
...
template <typename Elem> struct ElemVec2Converter;

template <> struct ElemVec2Converter<cutlass::half_t> {
    using Vec2 = __half2;
    inline __device__ static Vec2 from_float2(float2 x) {
        return __float22half2_rn(x);
    }
};

template <> struct ElemVec2Converter<cutlass::bfloat16_t> {
...
// MMA_N)).
template <typename Layout> inline __device__ auto convert_layout_acc_rowcol(Layout acc_layout) {
...
inline __device__ void softmax_rescale_o(Tensor0& scores,
    ...
    if (Is_first) {
        ...
        Tensor acc_o_rowcol =
            make_tensor(acc_o.data(), flash::convert_layout_acc_rowcol(acc_o.layout()));
#pragma unroll
